...
namespace macdoc
{
	public class AppleDocMerger
	{
		public class Options
		{
			...
			// Where to find the root documentation tree i.e. where the monodoc xmls are stored
			// in case of the documentation originated from a zipball, this is where the content
			// of it has been extracted
			public string MonoDocBaseDir { get; set; }
		...
		public AppleDocMerger (Options options)
		{
			...
			//assemblyTypesLookup = GetTypesSafe (options.Assembly).ToDictionary (t => t.Name, t => t);
			
			if (string.IsNullOrEmpty (options.MonoDocBaseDir) || !Directory.Exists (options.MonoDocBaseDir))
				throw new ArgumentException ("Monodoc dir isn't valid", "options.MonoDocBaseDir");
		...
		{
			return Path.Combine (options.MonoDocBaseDir, t.Namespace, t.Name + ".xml");
			//return string.Format ("{0}/{1}/{2}.xml", options.MonoDocBaseDir, t.Namespace, t.Name);
		}
